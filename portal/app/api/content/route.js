...
export async function GET() {
  ...
  const mapVideo = (v) => ({ id: v.id, title: v.title, description: v.description ?? '', instructions: v.instructions ?? [], videoURL: v.video_url ?? '', thumbnailURL: v.thumbnail_url ?? null, duration: v.duration ?? '', category: v.category ?? 'Meal Prep', tags: v.tags ?? [], slides: v.slides ?? [], });
